...
- **~35–40 GB de espaço em disco.** `embedding/`, `recommender/` e `api/`
  cada um baixa PyTorch + o modelo de embedding na build (~9,4 GB por
  imagem) e não compartilham camadas entre si (dependências ligeiramente
  diferentes em cada `requirements.txt`). `crawler`/`ingestion`/`database`
  são bem mais leves (200–450 MB cada).
- Acesso à rede: `crawler` baixa de `biblioteca.cesar.school` e
  `drive.google.com`; os módulos de ML baixam pacotes Python e os pesos do
  modelo (`paraphrase-multilingual-MiniLM-L12-v2`) durante o `docker build`.
...
| [`embedding/`](embedding/README.md) | Gera um vetor de 384 dimensões por chunk com um modelo local multilíngue. | Python 3.12 + `sentence-transformers` | Batch (`run --rm`) |
